...
echo ""
rpm -e --nodeps mysql-libs &>/dev/null
package="gcc gcc-c++ lzma unzip gzip bzip2 cmake ncurses-devel bison libxml2-devel openssl-devel libevent-devel"
for i in $package
do
...
cmake \
-DCMAKE_INSTALL_PREFIX=$mariadb_base_dir \
-DMYSQL_UNIX_ADDR=$mariadb_sock \
-DMYSQL_DATADIR=$mariadb_data_dir \
-DSYSCONFDIR=$mariadb_conf_dir \
-DMYSQL_user_flag=$mariadb_user \
...
#########################################################
echo ""
cp -f $build_dir/mariadb-*/support-files/mysql.server /etc/init.d/mysql
...
#########################################################
cp -f $build_dir/mariadb-*/support-files/mysql-log-rotate /etc/logrotate.d
